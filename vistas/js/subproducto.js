$(document).ready(function() {
   ...
   tablaProductos = $('#tablaProductos').DataTable({    
           ...
           "bDeferRender": true,	
           "bServerSide": true,                
           "sAjaxSource": "modelos/serverside/subproducto.php",	
           "columnDefs": [ {
               "targets": -1,        
   ...
   $('#formproductos').submit(function(e){                         
      ...
      s_producto = $.trim($('#s_producto').val());
      nombre     = $.trim($('#nombre').val());
      precio     = $.trim($('#precio').val()); 
       
           $.ajax({
             url: "modelos/include/subproducto.php",
             type: "POST",
             datatype:"json",    
             data:  {id:id, s_producto:s_producto, nombre:nombre, precio:precio, opcion:opcion},    
   ...
   $(document).on("click", ".btnEditar", function(){		        
       ...
       s_producto  = fila.find('td:eq(1)').text();
       nombre      = fila.find('td:eq(2)').text();
       precio      = fila.find('td:eq(3)').text();
      
     
       $("#s_producto").val(s_producto); 
       $("#nombre").val(nombre);
       $("#precio").val(precio);
   ...
   $(document).on("click", ".btnBorrar", function(){
       ...
       opcion = 3; //eliminar  
       Swal.fire({
         title: '¿Desea eliminar Sub Producto?',
            ...
            $.ajax({
              ...
              success: function() {
               Swal.fire({
                  position: 'top',
                  icon: 'success',
                  title: 'Sub Producto eliminado con exito'
                })
